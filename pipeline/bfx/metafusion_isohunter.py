...
def run_isohunter_singularity(out_dir_abspath, genome_fasta=None, gene_info=None, gene_bed=None, recurrent_bedpe=None, ini_section='metafusion'):
    num_tools = config.param('metafusion', 'num_tools', type='int')

    cff_dir_abspath = os.path.join(out_dir_abspath, "fusions", "cff")
    metafusion_outdir_abspath = os.path.join(out_dir_abspath, "fusions", "metafusion_isohunter")

    cff = os.path.join("fusions", "cff", "merged.cff")
    cff_abspath = os.path.join(out_dir_abspath, "fusions", "cff", "merged.cff")
    code_dir="/hpf/largeprojects/ccmbio/mapostolides/MetaFusion.IsoHunter"
...
cd {code_dir}/scripts ;\\ 
singularity exec -B /home -B {code_dir} -B /tmp -B /localhd/tmp -B {cff_dir_abspath} -B {metafusion_outdir_abspath} -B {reference_file_dir} /hpf/largeprojects/ccmbio/mapostolides/MetaFusion/MetaFusion.simg \\
bash MetaFusion.IsoHunter.sh --outdir {metafusion_outdir_abspath} \\
  --cff {cff} \\
  --gene_bed {gene_bed} \\
  --fusion_annotator \\
...
#bash MetaFusion.IsoHunter.sh --outdir $outdir \
#                 --cff $cff  \
#                 --gene_bed $gene_bed \
#                 --annotate_exons \
#                 --fusion_annotator \
